...
const logo = d.createElement(
  ...
  d
    .createElement("img")
    .setAttribute({ class: "logo", src: "./logo.svg", alt: "INFC Logo" })
);
...
const menu = d.createElement(
  "div",
  d.createElement("div").setAttribute({ class: "nav" }),
  { class: "menu" }
);
...
const profileImg = d
  .createElement("img")
  .setAttribute({ alt: "profile picture", src: "./assets/img/user.svg" });
...
for (let x in menuItemsLists) {
  itemsLists.push(
    d.createElement("div", x, {
      onclick: [`window.location='#/${menuItemsLists[x]}'`],
    })
  );
}
...
header.onload = (lists = "") => {
  ...
  for (let x in lists) {
    itemsLists.push(
      d.createElement("div", x, {
        onclick: `window.location = '#/${lists[x]}'`,
      })
    );
  }
